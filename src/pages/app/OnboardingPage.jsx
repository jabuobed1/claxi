...
export default function OnboardingPage() {
  ...
  const [cardForm, setCardForm] = useState({ cardNumber: '', nickname: '', paystackAuthorizationCode: '' });
  ...
  const addCard = async (event) => {
    event.preventDefault();
    const next = await addPaymentMethod(user, cardForm);
    setUser((prev) => ({ ...prev, ...next }));
    setCardForm({ cardNumber: '', nickname: '', paystackAuthorizationCode: '' });
    setStatusMessage('Card added and tokenized for future Paystack charges.');
  };
      ...
      {role === 'student' ? (
        <>
          ...
          <SectionCard title="Payment methods (Paystack)">
            <form className="grid gap-4 md:grid-cols-3" onSubmit={addCard}>
              <FormField
                label="Card number"
                name="cardNumber"
              ...
              <FormField
                ...
                onChange={(event) => setCardForm((prev) => ({ ...prev, nickname: event.target.value }))}
                placeholder="My Main Card"
                required
              />
              <FormField
                label="Paystack auth reference"
                name="paystackAuthorizationCode"
                value={cardForm.paystackAuthorizationCode}
                onChange={(event) => setCardForm((prev) => ({ ...prev, paystackAuthorizationCode: event.target.value }))}
                placeholder="Optional for now"
              />
              <div className="md:col-span-3">
                <button type="submit" className="rounded-2xl bg-brand px-4 py-2 text-sm font-bold text-white">Add card</button>
              </div>
            </form>
